...
public class TaskJob {
			...
			for (TBXX_TBRW tbrw : tbrwList) {
				...
				if (BMTZTG.endsWith(tbrw.getBM())) {
					...
					try {
						...
						System.out.println("同步表[" + tbrw.getBM() + "]的ID为["
								+ tbrw.getBID() + "]的数据发生错误");
	...
	private void DBSXSaveOrUpdate(TBXX_TBRW rw, DBZHJC_DBSX dbsx)
			throws AxisFault, RemoteException, WebServiceClientException {
	 ...
	 * @throws Exception
	 */
	private void webServiceUserAdd() throws WebServiceClientException,
			Exception {
		...
		int currPage = 1;
		// 获取版本号
		DateVersion dateVersion = dateVersionService
				.findById("com.nci.syncengine.wsbg.entity.SFSQ_YHZH");
		if (dateVersion == null) {
			DateVersion entity = new DateVersion();
			entity.setYwCode(SFSQ_YHZH.class.getName());
			dateVersion = dateVersionService.save(entity);
		}
		Long dv = dateVersion.getDataversion() == null ? 0 : dateVersion
				.getDataversion();
		Long version = dv;
		// 获取需要同步的user
	 ...
	 * @throws Exception
	 */
	private List<WebServiceUser> getUserList(CommonHttpWebServiceClient client,
			int currPage, Long dataVersion) throws Exception {
	 ...
	 * @throws WebServiceClientException
	 */
	private void addUserMap(List<WebServiceUser> userList)
			throws WebServiceClientException {
		...
		SyncAppUserMapHttpWebServiceClient client = new SyncAppUserMapHttpWebServiceClient(
				wsdl, username, password);
	...
	private List<WebServiceUser> _userList = new ArrayList<WebServiceUser>();
	@Autowired
